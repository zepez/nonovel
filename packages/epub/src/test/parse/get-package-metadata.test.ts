...
import { getPackageMetadata } from "../../lib/parse";
...
test("throws when metadata is missing or malformed", () => {
  expect(() => getPackageMetadata(getFilePath, getFileBuffer, "")).toThrowError(
    "Failed to get metadata"
  );

  expect(() =>
    getPackageMetadata(getFilePath, getFileBuffer, noMetaOpfContent)
  ).toThrowError("Failed to get metadata");
});

test("throws when title is not found", () => {
  expect(() =>
    getPackageMetadata(getFilePath, getFileBuffer, noTitleOpfContent)
  ).toThrowError("Failed to get title");
});

test("throws when creator is not found", () => {
  expect(() =>
    getPackageMetadata(getFilePath, getFileBuffer, noCreatorOpfContent)
  ).toThrowError("Failed to get creator");
});

test("returns object with with the correct shape", () => {
  const goodMeta = getPackageMetadata(
    getFilePath,
    getFileBuffer,
    goodOpfContent
  );
  ...
  const minMeta = getPackageMetadata(
    getFilePath,
    getFileBuffer,
    minimalOpfContent
  );
